...
class RollupPluginSpriteSmith {
	...
	start() {
		if (this.inited) Promise.resolve();
		this.inited = true;
		this.spinner = ora('sprite build...').start();
		return this.spriteStart().then(res => {
...
module.exports = function (config) {
	return {
		name: 'rollup-plugin-spritesmith',
		buildStart() {
			return new RollupPluginSpriteSmith(config).start();
		}
	}
};